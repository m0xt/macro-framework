...
## 2026-05-27T09:30:00Z — Task 54a iteration surface (engineer dispatch + bob hand-completion)
- What: Shipped `docs/index.html` iteration surface per ~/ops/PROJECT_SKELETON.md required section. Renderer at `src/macro_framework/build_index_page.py` imports MRMI math, posture bands, MMI driver specs, Growth Impulses inputs, Reference Library, release lags, and brief prompts directly from production code; wired into `scripts/refresh.sh` so it regenerates on every cron run alongside `outputs/dashboard.html`. Original Engineer dispatch hung mid-run on a gateway scope-upgrade / sandbox issue (see ~/ops/engineer/runs/2026-05-27T08-54-32Z-dispatch-macro-framework.log). Bob recovered Engineer's surviving artifacts and hand-completed the constants extraction in `macro_pipeline.py` and `weekly_briefs.py` since those edits were lost when discarding uncommitted partial work. Gates: `uv run ruff check .` ✅, `uv run pytest -q` (49 passed, 4 xfailed — three new renderer tests added) ✅, `git diff --check` ✅, renderer dry-run rebuilt page ✅.
- Files touched: src/macro_framework/macro_pipeline.py (added SECTOR_BREADTH_LOOKBACK, SECTOR_BREADTH_RECONCILIATION_NOTE, MMI_DRIVER_WEIGHTS, MMI_DRIVER_SPECS, MRMI_POSTURE_BANDS; refactored calc_sector_breadth to use the module constant); src/macro_framework/weekly_briefs.py (added PILLAR_BRIEF_USER_TEMPLATE, TOP_BRIEF_USER_TEMPLATE; refactored two call sites to .format() so the iteration surface and the actual prompt sent to Claude are single-source); src/macro_framework/build_index_page.py (NEW); docs/index.html (NEW); scripts/refresh.sh (wire build_index_page into cron, commit docs/index.html with outputs); tests/test_smoke.py (added build_index_page to PRODUCTION_MODULES + ENTRYPOINT_MODULES; rewrote test_breadth_lookback_docs_match_code to use module constant; added test_index_page_imports_iteration_surface_constants and test_index_page_reference_library_metadata_comes_from_dashboard_builder); tests/test_supabase_sync.py (updated mock harness for new entry point); .engineer/progress.md
- Commit: pending (this commit)
- Status: completed
