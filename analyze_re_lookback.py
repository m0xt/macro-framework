#!/usr/bin/env python3
"""
analyze_re_lookback.py

Tests different Real Economy z-score lookback windows (3y / 5y / 10y) holding
Inflation Δ at the empirically-optimal 6m. For each (RE lookback), runs the
conditioning backtest and shows:
...
def main():
    ...
    for lb_years in [3, 5, 10]:
        macro = calc_macro_context(data, lookback_years=lb_years)
        ...
        print("═" * 90)
        print(f"REAL ECONOMY z-score LOOKBACK: {lb_years}y    (n={len(df)} flips classified)")
        print("─" * 90)
        ...
        min_n = min((bucket_ns.get(q[0], 999) for q in quadrants), default=0)

        summary_rows.append({
            "lookback": f"{lb_years}y",
    ...
    print("═" * 90)
    print("CROSS-LOOKBACK SUMMARY · which RE lookback maximizes the conditioning effect?")
    print("─" * 90)
    print(f"{'Lookback':<10} {'Reflation 90d':>16} {'Stagflation 90d':>17} {'Gap':>10} {'Ref hit':>10} {'Stag hit':>10} {'min N':>6}")
    print("─" * 90)
