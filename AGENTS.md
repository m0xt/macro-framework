...
`macro-framework` ingests Yahoo/FRED macro and market data, computes the MRMI regime signal from market momentum plus macro stress (growth impulse, financial conditions, sector breadth, macro context, stress buffer), writes daily snapshots and a self-contained dashboard, syncs hot fields and backtest stats (single-row `macro_backtest` table) to Supabase, and lazily refreshes three Claude-generated weekly briefs on the first successful build on/after Tuesday.
...
| `scripts/refresh.sh` | Hermes Desktop cron refresh entry point via `~/ops/lib/cron-wrapper.sh`. |
| `scripts/com.milkroad.macro-refresh*.plist` | Legacy launchd templates retained for fallback; Hermes Desktop cron is the schedule authority. |
...
- Cron path: Hermes Desktop runs `scripts/refresh.sh` daily via `macro-refresh-daily` (`0 22 * * *`) and `scripts/refresh.sh --briefs-only` Monday via `macro-refresh/briefs` (`5 22 * * 1`). `scripts/refresh-if-et-time.sh` is kept for dry-runable ET gate checks and legacy launchd use.
...
- Manual build brief cadence is lazy weekly Tuesday; production forces Monday briefs after the daily data/dashboard refresh.
